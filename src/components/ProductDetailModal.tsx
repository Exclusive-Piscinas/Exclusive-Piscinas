                ...
                <div className="flex items-start justify-between">
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-4">
                      {product.category && (
                        <Badge className="bg-accent/20 text-accent border-accent/30 px-4 py-2 text-sm font-semibold backdrop-blur-sm">
                          {product.category.name}
                        </Badge>
                      )}
                      {product.featured && (
                        <Badge className="bg-accent text-accent-foreground border-accent px-4 py-2 text-sm font-semibold shadow-glow">
                  ...
                  <div className="flex items-center gap-3">
                    <Button
                      ...
                      className="text-primary-foreground hover:bg-primary-foreground/20 rounded-full w-12 h-12 transition-all duration-300"
                    >
                  ...
                  <div className="aspect-square relative overflow-hidden rounded-2xl bg-gradient-card shadow-elegant border border-border/30">
                    {allImages.length > 0 ? (
                      <>
                        ...
                        <div className="absolute inset-0 bg-gradient-to-t from-background/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500">
                          <div className="absolute top-6 right-6 flex gap-3">
                            <Button
                              ...
                              className="bg-card/90 hover:bg-card text-foreground rounded-xl shadow-elegant backdrop-blur-sm border border-border/30"
                            >
                        ...
                        {allImages.length > 1 && (
                          <>
                            <Button
                              ...
                              variant="secondary"
                              onClick={prevImage}
                              className="absolute left-6 top-1/2 -translate-y-1/2 bg-card/90 hover:bg-card text-foreground rounded-xl shadow-elegant backdrop-blur-sm border border-border/30 opacity-0 group-hover:opacity-100 transition-all duration-300"
                            >
                            ...
                            <Button
                              ...
                              variant="secondary"
                              onClick={nextImage}
                              className="absolute right-6 top-1/2 -translate-y-1/2 bg-card/90 hover:bg-card text-foreground rounded-xl shadow-elegant backdrop-blur-sm border border-border/30 opacity-0 group-hover:opacity-100 transition-all duration-300"
                            >
                        ...
                        {allImages.length > 1 && (
                          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 bg-background/80 text-foreground px-4 py-2 rounded-xl text-sm font-medium backdrop-blur-sm border border-border/30">
                        ...
                        <button
                          ...
                          className={`flex-shrink-0 w-24 h-24 rounded-xl overflow-hidden border-2 transition-all duration-300 ${
                            currentImageIndex === index 
                              ? 'border-accent shadow-glow scale-110 ring-2 ring-accent/30' 
              ...
              <div className="space-y-8">
                ...
                <Tabs defaultValue="description" className="w-full">
                  <TabsList className="grid w-full grid-cols-3 bg-muted/50 p-1 rounded-xl h-16 border border-border/30">
                    <TabsTrigger 
                      value="description" 
                      className="data-[state=active]:bg-card data-[state=active]:text-foreground data-[state=active]:shadow-elegant rounded-lg transition-all duration-300 font-semibold flex items-center gap-2"
                    >
                      ...
                      <div className="prose prose-invert max-w-none">
                        {product.description ? (
                          <div 
                            ...
                            className="text-foreground leading-relaxed [&_p]:mb-4 [&_h1]:text-foreground [&_h2]:text-foreground [&_h3]:text-foreground [&_strong]:text-accent [&_em]:text-accent" 
                          />
                        ) : (
                      ...
                      {product.features && product.features.length > 0 ? (
                        <div className="grid grid-cols-1 gap-4">
                          {product.features.map((feature, index) => (
                            <div 
                              key={index} 
                              className="flex items-center gap-4 p-6 bg-card/50 rounded-xl border border-border/20 hover:shadow-glow transition-all duration-300 group"
                            >
                        ...
                        <div className="space-y-6">
                          {productAccessories.map(accessory => {
                            ...
                            return (
                              <Card 
                                key={accessory.id} 
                                className={`transition-all duration-300 border-2 hover:shadow-elegant ${
                                  isSelected 
                                    ? 'border-accent shadow-glow bg-gradient-card' 
                                    : 'border-border/30 hover:border-accent/50 bg-card/30'
                                }`}
                              >
                                <CardContent className="p-6">
                                  <div className="flex items-start gap-6">
                                    ...
                                    {accessory.image_url && (
                                      <div className="w-24 h-24 rounded-xl overflow-hidden flex-shrink-0 border-2 border-border/30 shadow-card">
                                        <LazyImage 
                                    ...
                                    <div className="flex-1 space-y-4">
                                      <div className="flex items-center gap-3">
                                        ...
                                        {accessory.required && (
                                          <Badge className="bg-destructive/20 text-destructive border-destructive/30 text-xs font-semibold">
                                            Obrigatório
                                          </Badge>
                                        )}
                                      </div>
                                        ...
                                        {isSelected && (
                                          <div className="flex items-center gap-3 bg-muted/50 rounded-xl p-3 border border-border/30">
                                            <Button 
                                              ...
                                              className="w-10 h-10 p-0 hover:bg-accent hover:text-accent-foreground hover:border-accent rounded-lg"
                                            >
                ...
                <div className="bg-gradient-primary p-8 rounded-2xl shadow-elegant border border-primary/30">
                  <div className="space-y-6">
                    ...
                    <Button 
                      ...
                      className="w-full bg-accent hover:bg-accent-glow text-accent-foreground text-xl py-6 rounded-xl shadow-glow font-bold transition-all duration-300 hover:scale-105 border border-accent/30" 
                      size="lg"
                    >
          ...
          <DialogContent className="max-w-6xl max-h-[95vh] p-0 bg-background/95 border-0 backdrop-blur-sm">
            <div className="relative w-full h-full flex items-center justify-center p-8">
              <Button
                ...
                className="absolute top-6 right-6 text-foreground hover:bg-card/80 rounded-xl z-10 w-12 h-12"
              >
                <X className="w-6 h-6" />
              </Button>
              ...
              {allImages.length > 0 && (
                <img
                  src={allImages[currentImageIndex]}
                  alt={product.name}
                  className="max-w-full max-h-full object-contain rounded-xl shadow-elegant"
                />
              )}
              ...
              {allImages.length > 1 && (
                <>
                  <Button
                    ...
                    variant="ghost"
                    onClick={prevImage}
                    className="absolute left-6 top-1/2 -translate-y-1/2 text-foreground hover:bg-card/80 rounded-xl w-12 h-12"
                  >
                  ...
                  <Button
                    ...
                    variant="ghost"
                    onClick={nextImage}
                    className="absolute right-6 top-1/2 -translate-y-1/2 text-foreground hover:bg-card/80 rounded-xl w-12 h-12"
                  >
